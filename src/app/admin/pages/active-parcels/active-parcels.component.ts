...
currentPage: number = 1;
  ...
  // Modify loadParcels to support page changes
  loadParcels(type: string, page: number = 1): void {
    this.isLoading = true;
    this.currentPage = page;
    
    // Ensure your parcelService.getParcels accepts (type, page, limit)
    this.parcelService.getParcels(type, this.currentPage, this.pageSize).subscribe({
      next: (res: any) => {
  ...
  onPageChange(newPage: number): void {
    if (newPage >= 1 && newPage <= this.totalPages) {
      this.loadParcels(this.currentRouteStatus, newPage);
    }
  }
  constructor(
...
isActivestaus():boolean{
  return this.currentRouteStatus === 'active';
}
    buildYearList(): void {
